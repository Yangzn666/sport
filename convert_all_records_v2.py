    ...
    if line and '.' in line and len(line) == 10:  # 格式如 2026.05.18
        try:
            ...
            if 'km' in distance_str and 'kcal' in kcal:
                ...
                # 配速转换 - 修复解析逻辑
                # 原始格式如: 5'14''m:s/km 或 7'34''m:s/km
                pace_clean = pace_str.replace("m:s/km", "").replace("''", "'").strip()
                
                # 匹配分'秒" 格式
                import re
                match = re.search(r"(\d+)'(\d+)", pace_clean)
                if match:
                ...
                if distance >= 20:
                    ...
                    run_type = '长距离'
                elif pace_decimal > 0 and pace_decimal < 4.5:
                    run_type = '间歇跑'
                elif pace_decimal > 0 and pace_decimal < 5.0:
                    run_type = '节奏跑'
...
if records:
    ...
    print(f'配速示例: {[r["pace"] for r in records[:5]]}')
