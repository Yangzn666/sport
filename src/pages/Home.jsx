        ...
        <div className="max-w-7xl mx-auto px-6 py-12 md:py-16">
          ...
          <div className="max-w-2xl mx-auto mb-8 md:mb-12 px-4">
            <div className="relative">
              <input
                type="text"
                placeholder="搜索知识点（如：生物力学、肌肥大、运动营养...）"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full px-4 md:px-6 py-3 md:py-4 pr-12 text-base md:text-lg bg-white border-2 border-slate-300 rounded-xl focus:border-blue-600 focus:outline-none transition-colors shadow-sm"
              />
              <svg className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {filteredCategories.map((cat) => (
            <div key={cat.id} className="group bg-white rounded-xl border-2 border-slate-200 overflow-hidden hover:border-blue-600 hover:shadow-lg transition-all duration-300">
              <div className="p-6 md:p-8">
                ...
                <div className="flex gap-2 md:gap-3">
                  <Link 
                    to={`/knowledge/${cat.id}`} 
                    className="flex-1 inline-flex items-center justify-center px-3 md:px-5 py-2.5 md:py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium text-sm md:text-base shadow-sm"
                  >
                  ...
                  <Link 
                    to={`/category/${cat.id}`} 
                    className="inline-flex items-center px-3 md:px-5 py-2.5 md:py-3 bg-white border-2 border-slate-300 text-slate-700 rounded-lg hover:bg-slate-50 hover:border-slate-400 transition-colors font-medium text-sm md:text-base"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="mr-1.5 md:mr-2">
